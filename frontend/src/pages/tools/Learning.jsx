...
const generateTrendingTopics = () => [
  { name: "Financial Leak Detection", count: 342, icon: AlertTriangle },
  { name: "Money Flow Analysis", count: 289, icon: LineChart },
  { name: "Bad Debt Capitalization", count: 421, icon: DollarSign },
  { name: "AI Fraud Detection", count: 278, icon: Brain },
  { name: "Account Mismatch Forensics", count: 195, icon: Scale },
  { name: "Crypto Money Laundering", count: 156, icon: Lock }
];
...
const CommunityStats = () => {
  ...
  const textColor = themeMode === 'dark' ? 'text-white' : 'text-[#1B4332]';
  const secondaryColor = themeMode === 'dark' ? 'text-gray-300' : 'text-gray-500';
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <div className="text-center">
        ...
        <div className={`text-xs ${secondaryColor}`}>Total Members</div>
      </div>
      <div className="text-center">
        <div className={`text-lg font-bold ${textColor}`}>
          {online}
        </div>
        <div className={`text-xs ${secondaryColor}`}>Online Now</div>
      </div>
    </div>
  );
};
...
export default function Learning() {
  ...
  const textTertiaryColor = isDark ? 'text-gray-400' : 'text-gray-500';
  const borderColor = isDark ? 'border-white/20' : 'border-[#1B4332]/20';
  const cardBg = isDark ? 'bg-gray-800' : 'bg-white';
  const hoverBg = isDark ? 'hover:bg-white/5' : 'hover:bg-[#1B4332]/5';
  const buttonBg = isDark ? 'bg-[#1B4332] hover:bg-[#2D6A4F]' : 'bg-[#1B4332] hover:bg-[#2D6A4F]';
  const buttonOutline = isDark ? 'border-white/20 text-white hover:bg-white/10' : 'border-[#1B4332]/20 text-[#1B4332] hover:bg-[#1B4332]/10';
  const iconColor = isDark ? 'text-[#1B4332]' : 'text-[#1B4332]';
  const badgeLightBg = isDark ? 'bg-white/10' : 'bg-[#1B4332]/10';
  const badgeLightText = isDark ? 'text-white' : 'text-[#1B4332]';
  const badgeDarkBg = isDark ? 'bg-white/5' : 'bg-[#1B4332]/5';
  const badgeDarkText = isDark ? 'text-gray-300' : 'text-[#1B4332]';
  const progressBg = isDark ? 'bg-white/10' : 'bg-[#D8F3DC]';
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
          <Card className={`lg:col-span-3 ${cardBg}`}>
            <CardContent className="p-6">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div>
                  <h1 className={`text-3xl font-bold ${textColor} mb-2`}>Financial Intelligence Command Center</h1>
                  <p className={`${textSecondaryColor} mt-1`}>Master the art of detecting financial leaks, tracing money flows, and capitalizing on distressed assets</p>
                </div>
                <div className="flex items-center gap-2">
                  <Badge className={`${badgeLightBg} ${badgeLightText} border ${borderColor}`}>
                    <CheckCircle className="h-3 w-3 mr-1" />
                    {data.userProgress?.certificationsEarned || 0} Certifications
                  </Badge>
                  <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#2D6A4F]/10 text-[#2D6A4F]'} border ${isDark ? 'border-white/20' : 'border-[#2D6A4F]/20'}`}>
                    <Clock className="h-3 w-3 mr-1" />
                    {data.userProgress?.totalLearningHours || 0}h Learned
                  </Badge>
                  <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#40916C]/10 text-[#40916C]'} border ${isDark ? 'border-white/20' : 'border-[#40916C]/20'}`}>
          ...
          <Card className={cardBg}>
            <CardContent className="p-6">
              <div className="text-center">
                ...
                <div className={`text-sm ${textSecondaryColor}`}>Modules Completed</div>
                <Progress value={(data.userProgress?.completedCourses || 0) * 10} className={`mt-2 ${progressBg}`} />
                <div className={`text-xs ${textTertiaryColor} mt-1`}>
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription className={textSecondaryColor}>
                  Advanced training in financial crime detection, money flow analysis, and asset recovery
                </CardDescription>
              </div>
              <Button variant="outline" size="sm" className={buttonOutline}>
                View All Paths
              </Button>
            </div>
          </CardHeader>
              ...
              {data.learningPaths.map((path) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`border ${borderColor} rounded-xl p-4 space-y-3 hover:shadow-lg transition-shadow ${cardBg}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`p-2 ${badgeDarkBg} rounded-lg`}>
                          <IconComponent className={`h-5 w-5 ${iconColor}`} />
                        </div>
                        <Badge className={
                          path.badge === "Premium" ? `${isDark ? 'bg-white/10 text-white' : 'bg-[#1B4332]/10 text-[#1B4332]'} border ${isDark ? 'border-white/20' : 'border-[#1B4332]/20'}` : 
                          path.badge === "Popular" ? `${isDark ? 'bg-white/10 text-white' : 'bg-[#2D6A4F]/10 text-[#2D6A4F]'} border ${isDark ? 'border-white/20' : 'border-[#2D6A4F]/20'}` : 
                          `${isDark ? 'bg-white/10 text-white' : 'bg-[#40916C]/10 text-[#40916C]'} border ${isDark ? 'border-white/20' : 'border-[#40916C]/20'}`
                        }>
                          {path.badge}
                        </Badge>
                      </div>
                      <Bookmark 
                        className={`h-5 w-5 cursor-pointer ${bookmarked.has(path.id) ? `${iconColor} ${isDark ? 'fill-white/20' : 'fill-[#1B4332]/20'}` : 'text-gray-400'}`}
                        onClick={() => handleBookmark(path.id)}
                      />
                    </div>
                    
                    <h3 className={`font-bold text-lg ${textColor}`}>{path.title}</h3>
                    <p className={`${textSecondaryColor} text-sm`}>{path.description}</p>
                    
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className={textTertiaryColor}>Progress</span>
                        <span className={`font-medium ${textColor}`}>{userProgress}%</span>
                      </div>
                      <Progress value={userProgress} className={progressBg} />
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <CardContent className="p-4">
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="relative flex-1">
                    <Search className={`absolute left-3 top-1/2 -translate-y-1/2 ${isDark ? 'text-gray-400' : 'text-gray-400'}`} />
                    <input
                      value={searchText}
                      onChange={(e) => setSearchText(e.target.value)}
                      placeholder="Search financial crime, money flow, bad debt, AI detection..."
                      className={`w-full pl-10 py-2 border ${borderColor} rounded-lg focus:ring-2 focus:ring-[#1B4332]/30 focus:border-[#1B4332] bg-transparent ${textColor}`}
                    />
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className={`gap-2 ${buttonOutline}`}>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className={`border ${borderColor} rounded-xl p-4 space-y-3`}>
                    ...
                    <h4 className={`font-semibold ${textColor}`}>Subscribe to Financial Intelligence</h4>
                    <p className={`text-sm ${textSecondaryColor}`}>Advanced tutorials on detecting financial leaks, tracing money flows, and capitalizing on distressed debt</p>
                    <div className="flex items-center justify-between">
                      <span className={`text-sm ${textTertiaryColor}`}>15K+ Financial Analysts</span>
                      <Button asChild className={`gap-2 ${buttonBg} text-white`}>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className={`font-semibold ${textColor}`}>Featured Intelligence Videos</h4>
                      <Button variant="link" size="sm" className={`gap-1 ${textColor} hover:text-[#2D6A4F]`} asChild>
                        <a href={YOUTUBE_CHANNEL} target="_blank" rel="noopener noreferrer">
                          View All
                    ...
                    {data.videos.map((video) => (
                      <div key={video.id} className={`flex items-center gap-3 p-2 ${hoverBg} rounded-lg border ${borderColor}`}>
                        ...
                        <div className="flex-1">
                          <p className={`text-sm font-medium ${textColor}`}>{video.title}</p>
                          <p className={`text-xs ${textTertiaryColor}`}>{video.duration} • {video.views} views</p>
                          {video.started && (
                            <div className="mt-1">
                              <div className="flex justify-between text-xs text-gray-500 mb-1">
                                <span className={textTertiaryColor}>Progress</span>
                                <span className={textColor}>{video.progress}%</span>
                              </div>
                              <Progress value={video.progress} className={`h-1 ${progressBg}`} />
                            </div>
                          )}
                        </div>
                        <Button 
            ...
            <Card className={cardBg}>
              <CardHeader>
                <div className="flex justify-between items-center">
                  <div>
                    <CardTitle className={`flex items-center gap-2 ${textColor}`}>
                      <BookOpen className={`h-6 w-6 ${iconColor}`} />
              ...
              <CardContent className="space-y-4">
                {loading && (
                  <div className="space-y-4">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className={`animate-pulse border rounded-xl p-5 h-32 ${isDark ? 'bg-white/5' : 'bg-[#1B4332]/5'}`}></div>
                    ))}
                  </div>
                )}
                ...
                {!loading && filteredArticles.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className={textSecondaryColor}>No intelligence briefs match your search criteria.</p>
                  </div>
                )}
                ...
                {filteredArticles.map((article) => (
                  <motion.div
                    ...
                    className={`border ${borderColor} rounded-xl p-5 space-y-3 hover:shadow-md transition-shadow ${cardBg}`}
                  >
                    <div className="flex justify-between items-start">
                      <div className="space-y-2 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {article.premium && (
                            <Badge className={`${badgeLightBg} ${badgeLightText} border ${borderColor}`}>Premium</Badge>
                          )}
                          {article.featured && (
                            <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#2D6A4F]/10 text-[#2D6A4F]'} border ${isDark ? 'border-white/20' : 'border-[#2D6A4F]/20'}`}>Featured</Badge>
                          )}
                          {article.completed && (
                            <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#40916C]/10 text-[#40916C]'} border ${isDark ? 'border-white/20' : 'border-[#40916C]/20'}`}>Completed</Badge>
                          )}
                          {article.started && !article.completed && (
                            <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#52B788]/10 text-[#52B788]'} border ${isDark ? 'border-white/20' : 'border-[#52B788]/20'}`}>In Progress</Badge>
                          )}
                        </div>
                        <p className={textSecondaryColor}>{article.summary}</p>
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                          ...
                          <Badge className={`${badgeDarkBg} ${badgeDarkText} border ${borderColor}`}>{article.category}</Badge>
                          <span className="flex items-center gap-1">
                      ...
                      <div className="flex flex-col items-center gap-2 ml-4">
                        <Bookmark 
                          className={`h-5 w-5 cursor-pointer ${bookmarked.has(article.id) ? `${iconColor} ${isDark ? 'fill-white/20' : 'fill-[#1B4332]/20'}` : 'text-gray-400'}`}
                        ...
                        <Button
                          ...
                          className={`h-8 w-8 ${isDark ? 'hover:bg-white/10' : 'hover:bg-[#1B4332]/10'}`}
                        >
                          <ThumbsUp className="h-4 w-4" />
                        </Button>
                        <span className={`text-xs ${textColor}`}>{getEngagementCount(article.id, 'likes')}</span>
                      </div>
                    </div>
                    ...
                    <div className={`flex justify-between items-center pt-2 border-t ${borderColor}`}>
                      <div className="flex items-center gap-4">
                        <Button
                          variant="ghost"
                          size="sm"
                          className={`gap-2 ${isDark ? 'hover:bg-white/10' : 'hover:bg-[#1B4332]/10'} ${textColor}`}
                          onClick={() => handleEngagement(article.id, 'comments')}
                        >
                          <MessageSquare className="h-4 w-4" />
                          Comment
                        </Button>
                        <Button variant="ghost" size="sm" className={`gap-2 ${isDark ? 'hover:bg-white/10' : 'hover:bg-[#1B4332]/10'} ${textColor}`}>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button
                          className={`${buttonBg} text-white gap-2`}
                          onClick={() => {
          ...
          <div className="space-y-6">
            ...
            <Card className={cardBg}>
              ...
              <CardContent className="space-y-3">
                {data.certifications.map((cert) => (
                  <div key={cert.id} className={`border ${borderColor} rounded-lg p-3 ${hoverBg} transition-colors`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className={`font-semibold text-sm ${textColor}`}>{cert.title}</h4>
                        <p className={`text-xs ${textTertiaryColor}`}>{cert.issuer}</p>
                      </div>
                      <Badge className={`text-xs border ${cert.badgeColor} ${isDark ? 'bg-white/5 text-white' : ''}`}>
                        {cert.difficulty}
                      </Badge>
                    </div>
                    <div className="flex justify-between items-center mt-2">
                      <span className={`text-xs ${textTertiaryColor}`}>{cert.duration}</span>
                      {cert.enrolled ? (
                        cert.completed ? (
                          <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#40916C]/10 text-[#40916C]'} border ${isDark ? 'border-white/20' : 'border-[#40916C]/20'} text-xs`}>
                            Completed
                          </Badge>
                        ) : (
                          <div className="flex items-center gap-2">
                            <Progress value={cert.progress} className={`w-16 h-2 ${progressBg}`} />
                            <span className={`text-xs ${textColor}`}>{cert.progress}%</span>
                          </div>
                        )
                      ) : (
                        <Button 
                          size="sm" 
                          className={`text-xs ${buttonBg} text-white`}
            ...
            <Card className={cardBg}>
              ...
              <CardContent className="space-y-3">
                {data.liveSessions.map((session) => (
                  <div key={session.id} className={`border ${borderColor} rounded-lg p-3 ${hoverBg} transition-colors`}>
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className={`font-semibold text-sm ${textColor}`}>{session.title}</h4>
                        <p className={`text-xs ${textTertiaryColor}`}>Host: {session.host}</p>
                      </div>
                      {session.registered && (
                        <Badge className={`${isDark ? 'bg-white/10 text-white' : 'bg-[#2D6A4F]/10 text-[#2D6A4F]'} border ${isDark ? 'border-white/20' : 'border-[#2D6A4F]/20'} text-xs`}>
                          {session.attended ? 'Attended' : 'Registered'}
                        </Badge>
                      )}
                    </div>
                    <div className="flex justify-between items-center mt-2">
                      <span className={`text-xs ${textTertiaryColor}`}>
                      ...
                      <span className={`text-xs ${textTertiaryColor} flex items-center gap-1`}>
                        <Users className="h-3 w-3" />
                        {session.attendees}
                      </span>
                    </div>
                    {!session.registered && (
                      <Button 
                        size="sm" 
                        className={`w-full mt-2 ${buttonBg} text-white`}
            ...
            <Card className={cardBg}>
              ...
              <CardContent className="space-y-2">
                {data.trendingTopics.map((topic, index) => {
                  ...
                  return (
                    <div key={topic.name} className={`flex items-center justify-between p-2 ${hoverBg} rounded border border-transparent hover:border ${borderColor}`}>
                      <div className="flex items-center gap-2">
                        <div className={`text-lg font-semibold ${isDark ? 'text-white/30' : 'text-[#1B4332]/30'}`}>#{index + 1}</div>
                        <div className="flex items-center gap-2">
                          <IconComponent className={`h-4 w-4 ${iconColor}`} />
                          <span className={`text-sm ${textColor}`}>{topic.name}</span>
                        </div>
                      </div>
                      <Badge variant="outline" className={`border ${borderColor} ${textColor}`}>{topic.count} analyses</Badge>
                    </div>
                  );
                })}
              </CardContent>
            ...
            <Card className={cardBg}>
              ...
              <CardContent className="space-y-3">
                {/* Discord Community */}
                <div className={`flex items-center justify-between p-2 ${hoverBg} rounded-lg transition-colors border border-transparent hover:border ${borderColor}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 ${badgeDarkBg} rounded-lg flex items-center justify-center`}>
                    ...
                    <div>
                      <div className={`font-medium text-sm ${textColor}`}>Financial Crime Discord</div>
                      <div className={`text-xs ${textTertiaryColor}`}>Real-time intelligence sharing</div>
                    </div>
                  </div>
                  <Button 
                    size="sm" 
                    variant="outline"
                    className={`text-xs ${buttonOutline}`}
                    asChild
                  >
                    <a 
                ...
                <div className={`flex items-center justify-between p-2 ${hoverBg} rounded-lg transition-colors border border-transparent hover:border ${borderColor}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className={`font-medium text-sm ${textColor}`}>Financial Intelligence Network</div>
                      <div className={`text-xs ${textTertiaryColor}`}>Professional networking</div>
                    </div>
                  </div>
                  <Button 
                    size="sm" 
                ...
                <div className={`flex items-center justify-between p-2 ${hoverBg} rounded-lg transition-colors border border-transparent hover:border ${borderColor}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className={`font-medium text-sm ${textColor}`}>Forensic Analysis Forum</div>
                      <div className={`text-xs ${textTertiaryColor}`}>Q&A with financial crime experts</div>
                    </div>
                  </div>
                  <Button 
                    size="sm" 
                ...
                <div className={`flex items-center justify-between p-2 ${hoverBg} rounded-lg transition-colors border border-transparent hover:border ${borderColor}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className={`font-medium text-sm ${textColor}`}>Weekly Intelligence Report</div>
                      <div className={`text-xs ${textTertiaryColor}`}>Market anomalies & financial crime insights</div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Button 
                ...
                <div className={`pt-3 border-t ${borderColor} mt-2`}>
                  <CommunityStats />
                </div>
